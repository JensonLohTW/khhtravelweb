...
## 功能摘要
- 透過 `app-files/config/content-overrides.json` 集中管理場景名稱與資訊熱點文案，部署時自動套用，確保客戶端與開發環境內容一致。
...
2. 左側場景列表雙擊需要調整的名稱，或直接編輯 `config/content-overrides.json` 中的文案。
...
- 若需更嚴謹的校驗，可在 CI/部署腳本中加入對 `content-overrides.json` 的 `hash` 或 `version` 比對，確保與預期版本一致。
...
- **熱點索引難以辨識？** 可在開發工具中 `console.log(window.APP_DATA.scenes)` 檢視每個場景的 `infoHotspots` 順序。
